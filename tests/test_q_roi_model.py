...
class TestGetRoi:
    def test_get_roi_valid_index(self, model: QROIModel) -> None:
        roi = _make_roi("A")
        model.addROI(roi)
        assert model.getRoi(0) is roi

    def test_get_roi_multiple(self, model: QROIModel) -> None:
    ...
    def test_get_roi_out_of_bounds(self, model: QROIModel, index: int) -> None:
        with pytest.raises(IndexError, match="Index out of bounds"):
            model.getRoi(index)

    def test_get_roi_after_remove(self, model: QROIModel) -> None:
        roi_a = _make_roi("A")
        roi_b = _make_roi("B")
        model.addROI(roi_a)
        model.addROI(roi_b)

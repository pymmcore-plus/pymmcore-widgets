...
def test_reception(qtbot):
    ...
    datastore = QLocalDataStore(shape=[3, 1, 1, 512, 512], dtype=np.uint16, mmcore=mmcore)
...
def test_index_completion():
    mmcore = CMMCorePlus.instance()
    datastore = QLocalDataStore(shape=[10, 1, 1, 512, 512], dtype=np.uint16, mmcore=mmcore)

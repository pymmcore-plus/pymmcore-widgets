...
class LabeledSlider(QWidget):
    ...
    def setValue(self, value: float | str) -> None:
        ...
        with _update_blocker(self):
            ...
            if self._is_float:
                self._slider.setValue(int(typed_val * self._scale))
            else:
                self._slider.setValue(typed_val)
...
class PropertyWidget(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self._value_widget = create_inner_property_widget(
            self._mmc, device_label, prop_name
        )

        # Set initial value
        self._try_update_from_core()

        self._value_widget.valueChanged.connect(self._on_widget_changed)
    ...
    def _on_config_loaded(self) -> None:
        ...
        with contextlib.suppress(RuntimeError):
            ...
            if isinstance(self._value_widget, ChoiceComboBox):
                allowed = _get_allowed_values(self._mmc, self._device, self._prop)
                self._value_widget.setChoices(allowed)
            self._try_update_from_core()

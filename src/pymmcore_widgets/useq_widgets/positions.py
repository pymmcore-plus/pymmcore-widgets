...
class PositionTable(_DataTable):
    ...
    X = ColumnMeta(key="x", header="X [mm]", type=float, default=0)
    Y = ColumnMeta(key="y", header="Y [mm]", type=float, default=0)
    Z = ColumnMeta(key="z", header="Z [mm]", type=float, default=0)

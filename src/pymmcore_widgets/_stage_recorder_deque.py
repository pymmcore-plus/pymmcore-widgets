...
class StageRecorder(QWidget):
    ...
    def _on_sequence_starded(self) -> None:
        ...
        self._worker = create_worker(
            self._watch_mda,
            _start_thread=True,
            _connect={"yielded": self._update_scene},
        )

    def _watch_mda(self) -> Generator[tuple[np.ndarray, MDAEvent], None, None]:
    ...
    # def _update_scene(self, image: np.ndarray, event: MDAEvent) -> None:
    def _update_scene(self, args: Any) -> None:
        image, event = args
        event = cast(MDAEvent, event)
    ...
    def _on_sequence_ended(self) -> None:
        ...
        while self._deck:
            self._update_scene(self._deck.popleft())
